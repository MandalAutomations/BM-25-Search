...
In this article, we'll show how automatic reviews from Copilot can help optimize your review process so you spend less time on minor changes and more time on nuanced problem solving and deeper understanding for implementation that's not simply adequate, but skillfully meets user needs.

## 1. Improve review quality from Copilot

Copilot code review can provide automated reviews for all pull requests in your repository and make reviewing more efficient by catching changes you don't want in your code. When paired with custom instructions, Copilot code review is more effective because it can provide responses that are tailored to the way your team works, the tools you use, or the specifics of your project.
...
In the next section, we'll walk through how our example custom instructions can positively impact the review comments Copilot provides for a pull request.

## 2. Automate reviews to address style conventions and performance

When automatic Copilot code review is enabled, Copilot will automatically review a pull request once it's out of draft mode.
...
Copilot would review the code and may make the following comment and code suggestion.
...
The response from Copilot follows guidance from your custom instructions in its recommendation. In this example, it follows the PEP 8 and 257 style guidance for Python and suggests an improvement to better handle timeouts and error logging, while concisely explaining the reasoning behind the suggestions.

> \[!NOTE] Always carefully review Copilot's suggestions before accepting and committing.
...
While Copilot code review can make security best practice suggestions, Copilot Autofix for code scanning takes it a step further. Leveraging the capabilities of code scanning with CodeQL analysis to analyze the code in a GitHub repository and find security vulnerabilities and coding errors, Copilot Autofix can then suggest fixes for alerts, enabling you to prevent and reduce vulnerabilities more efficiently.

For example, Copilot Autofix may make the following comment on the code.
...
Copilot Autofix will also make code suggestions for a potential fix to the vulnerability for you to review. In this case, it may make code suggestions, like those below, to import a package and update the code related to hashing the password.
...
> \[!NOTE]
>
> * Always verify and validate any changes Copilot suggests before accepting them.
> * In this example, Copilot code review may also highlight the need to generate unique salts.

As you can see, identifying vulnerabilities automatically, along with suggestions for fixing them, helps you make security a priority. Copilot Autofix enables you to focus on understanding secure coding and on fixes that work best for your code base and project.
...
* Custom instructions helped refine the responses from Copilot code review so they were specific to our project and user needs and we also saw how we can tailor how much explanation Copilot provides in feedback.
* Copilot code review  helped us quickly improve our error logging and understand why it mattered.
* Copilot Autofix for code scanning helped us prevent using an insufficient password hashing approach and protect user data.
...
1. Create custom instructions specific to your project and repository. Write your own, or take inspiration from our library of examples. See [Custom instructions](/en/copilot/tutorials/customization-library/custom-instructions).
2. To enable automatic Copilot code review for your repository, see [Configuring automatic code review by GitHub Copilot](/en/copilot/how-tos/copilot-on-github/set-up-copilot/configure-automatic-review).
3. To configure Copilot Autofix for your repo you'll need to enable code scanning. Once code scanning with CodeQL analysis is enabled, Copilot Autofix is enabled by default. For the easiest setup, see [Configuring default setup for code scanning](/en/code-security/code-scanning/enabling-code-scanning/configuring-default-setup-for-code-scanning).

## Further reading

To go deeper with reviewing AI generated code, see [Review AI-generated code](/en/copilot/tutorials/review-ai-generated-code).