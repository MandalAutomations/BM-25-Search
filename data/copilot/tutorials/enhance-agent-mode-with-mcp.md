...
* **Use OAuth when available**: For MCP servers like GitHub MCP, prefer OAuth authentication over personal access tokens. See [Using the GitHub MCP Server in your IDE](/en/copilot/customizing-copilot/using-model-context-protocol/using-the-github-mcp-server#remote-mcp-server-configuration-with-oauth).
