...
Connecting GitHub Copilot SDK to your locally signed-in CLI is the fastest way to get started.

**Best for:** Personal projects, prototyping, local development, and learning the SDK.

## How it works

When you install Copilot CLI and sign in, your credentials are stored in the system keychain. The SDK automatically starts the CLI as a child process and uses those stored credentials. Key characteristics:
...
Replace `YOUR-GITHUB-TOKEN` with a valid GitHub personal access token or OAuth token. The SDK picks these up automatically—no code changes are needed.
...
* To ship your app to other users, see [Using a bundled CLI with Copilot SDK](/en/copilot/how-tos/copilot-sdk/set-up-copilot-sdk/bundled-cli).
* To support multiple users signing in with their own GitHub accounts, see [Using GitHub OAuth with Copilot SDK](/en/copilot/how-tos/copilot-sdk/set-up-copilot-sdk/github-oauth).
* To run the SDK on a server, see [Setting up Copilot SDK for backend services](/en/copilot/how-tos/copilot-sdk/set-up-copilot-sdk/backend-services).
* For your first message and installation, see [Getting started with Copilot SDK](/en/copilot/how-tos/copilot-sdk/sdk-getting-started).