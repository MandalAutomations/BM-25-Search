...
You can configure Copilot code review to review pull requests automatically. For an overview of automatic pull request reviews, see [About GitHub Copilot code review](/en/copilot/concepts/agents/code-review#automatic-pull-request-reviews).
...
4. To choose the depth of Copilot code reviews, next to "Review effort level," select the effort level for automatic reviews in this repository.
   ...
   * **Balanced**: Deeper analysis of complex logic, security-sensitive code, and cross-service changes.
   ...
   Balanced reviews use more AI credits, and may consume marginally more GitHub Actions minutes. See [About GitHub Copilot code review](/en/copilot/concepts/agents/code-review#estimated-consumption).
...
8. To activate the ruleset, under "Enforcement Status", select **Active**.
...
10. Type a pattern that matches the repository names you want to target—for example, `*feature` to match all repositories with names ending in `feature`.
    ...
    For pattern-matching syntax, see [Creating rulesets for repositories in your organization](/en/organizations/managing-organization-settings/creating-rulesets-for-repositories-in-your-organization#using-fnmatch-syntax).
...
13. Under "Target branches," click **Add target** and choose one of the target options.

14. Under "Branch rules," select **Automatically request Copilot code review**.

15. Optionally, if you want Copilot to review all new pushes to the pull request, select **Review new pushes**.

    If this option is not selected, Copilot will only review the pull request once.

16. Optionally, if you want Copilot to review pull requests while they are still drafts, select the **Review draft pull requests**.

    This can be a useful option for catching errors early, before requesting a human review.

17. At the bottom of the page, click **Create**.
