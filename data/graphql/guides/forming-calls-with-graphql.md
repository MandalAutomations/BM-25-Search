...
To authenticate with a personal access token, follow the steps in [Managing your personal access tokens](/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token). The data that you are requesting will dictate which scopes or permissions you will need.
...
Because GraphQL operations consist of multiline JSON, GitHub recommends using the [GraphQL Clients](/en/graphql/guides/using-graphql-clients) to make GraphQL calls. You can also use `curl` or any other HTTP-speaking library.

In REST, [HTTP verbs](/en/rest#http-verbs) determine the operation performed. In GraphQL, you'll provide a JSON-encoded body whether you're performing a query or a mutation, so the HTTP verb is `POST`. The exception is an [introspection query](/en/graphql/guides/introduction-to-graphql#discovering-the-graphql-api), which is a simple `GET` to the endpoint. For more information on GraphQL versus REST, see [Migrating from REST to GraphQL](/en/graphql/guides/migrating-from-rest-to-graphql).
...
The two types of allowed operations in GitHub's GraphQL API are *queries* and *mutations*. Comparing GraphQL to REST, queries operate like `GET` requests, while mutations operate like `POST`/`PATCH`/`DELETE`. The [mutation name](/en/graphql/reference/mutations) determines which modification is executed.

For information about rate limiting, see [Rate limits and query limits for the GraphQL API](/en/graphql/overview/resource-limitations).

Queries and mutations share similar forms, with some important differences.

### About queries

GraphQL queries return only the data you specify. To form a query, you must specify [fields within fields](/en/graphql/guides/introduction-to-graphql#field) (also known as *nested subfields*) until you return only [scalars](/en/graphql/reference/scalars).
...
In the [mutations](/en/graphql/reference/mutations) reference, the listed *input fields* are what you pass as the input object. The listed *return fields* are what you pass as the payload object.
...
* `repository(owner:"octocat", name:"Hello-World") {`
  ...
  To begin the query, we want to find a [`repository`](/en/graphql/reference/objects#repository) object. The schema validation indicates this object requires an `owner` and a `name` argument.
...
* `issues(last:20, states:CLOSED) {`
  ...
  * The [docs](/en/graphql/reference/objects#repository) tell us this object has the type `IssueConnection`.
  * Schema validation indicates this object requires a `last` or `first` number of results as an argument, so we provide `20`.
  * The [docs](/en/graphql/reference/objects#repository) also tell us this object accepts a `states` argument, which is an [`IssueState`](/en/graphql/reference/enums#issuestate) enum that accepts `OPEN` or `CLOSED` values. To find only closed issues, we give the `states` key a value of `CLOSED`.
...
* `node {`
  ...
  Here we retrieve the node at the end of the edge. The [`IssueConnection` docs](/en/graphql/reference/objects#issueconnection) indicate the node at the end of the `IssueConnection` type is an `Issue` object.

* Now that we know we're retrieving an `Issue` object, we can look at the [docs](/en/graphql/reference/objects#issue) and specify the fields we want to return:
  ...
  The `labels` field has the type [`LabelConnection`](/en/graphql/reference/objects#labelconnection). As with the `issues` object, because `labels` is a connection, we must travel its edges to a connected node: the `label` object. At the node, we can specify the `label` object fields we want to return, in this case, `name`.
...
Because we want to modify data on the server (attach an emoji to an issue), we begin by searching the schema for a helpful mutation. The reference docs show the [`addReaction`](/en/graphql/reference/mutations#addreaction) mutation, with this description: `Adds a reaction to a subject.` Perfect!
...
* `addReaction(input:{subjectId:"MDU6SXNzdWUyMzEzOTE1NTE=",content:HOORAY}) {`
  ...
  * `{subjectId:"MDU6SXNzdWUyMzEzOTE1NTE=",content:HOORAY}` is the required argument value. This will always be an [input object](/en/graphql/reference/input-objects) (hence the curly braces) composed of input fields (`subjectId` and `content` in this case) for a mutation.

  How do we know which value to use for the content? The [`addReaction` docs](/en/graphql/reference/mutations#addreaction) tell us the `content` field has the type [`ReactionContent`](/en/graphql/reference/enums#reactioncontent), which is an [enum](/en/graphql/reference/enums) because only certain emoji reactions are supported on GitHub issues. These are the allowed values for reactions (note some values differ from their corresponding emoji names):
...
* The rest of the call is composed of the payload object. This is where we specify the data we want the server to return after we've performed the mutation. These lines come from the [`addReaction` docs](/en/graphql/reference/mutations#addreaction), which three possible return fields:
...
>
> * When you use `content` directly in the mutation, the schema expects the value to be of type [`ReactionContent`](/en/graphql/reference/enums#reactioncontent), which is an *enum*, not a string. Schema validation will throw an error if you add quotes around the enum value, as quotes are reserved for strings.
