...
* GitHub CLI, with the `gh-stack` extension installed. The CLI extension is the most comprehensive way to create and manage stacks.
* Branch protection rules configured for your default branch.
* GitHub Actions workflows that run on pull requests targeting your default branch.

Copilot isn't required to use stacked pull requests, but is encouraged for teams that want to stack AI-generated changes. Stacked pull requests also work with other AI coding agents, such as Claude Code and Codex, using the provided agent skill. See [Stack AI-generated code in pull requests](/en/copilot/tutorials/stack-ai-generated-code-in-pull-requests).
...
### Reordering a stack requires GitHub CLI

If your team needs to reorder the pull requests in a stack, they will need to use `gh stack modify` in the `gh stack` GitHub CLI extension. There's no way to reorder a stack from the GitHub website. Teams who don't use the CLI locally should plan their stack order carefully upfront, or install the extension for this task.
...
* A GitHub Actions workflow that triggers on `pull_request` events targeting `main` runs for **every** pull request in the stack, so your existing CI configuration doesn't need to change.
* Stack metadata is available in workflow expressions via `github.event.pull_request.stack`, if you want to customize workflow behavior specifically for stacked pull requests. Since a workflow runs once per pull request in a stack, teams can use this metadata to skip expensive jobs on redundant runs and reduce CI usage. For details, see [Optimizing CI for stacked pull requests](/en/pull-requests/how-tos/merge-and-close-pull-requests/optimizing-ci-for-stacked-pull-requests).
...
2. Your CI workflows run on every pull request in the stack.
3. Merging is blocked until the pull request in the stack you want to merge, and everything below it, meets your requirements.

For the full list of rules and requirements, see [Stacked pull requests](/en/pull-requests/reference/stacked-pull-requests).

## 3. Get your teams started

Once you've reviewed the rollout considerations and understand how your rules and CI work with stacks, point your teams to [Stacked pull requests](/en/pull-requests/how-tos/stacked-pull-requests), which brings together everything they need to start creating, reviewing, and merging stacked pull requests.
