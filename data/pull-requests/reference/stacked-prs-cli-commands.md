...
| [`gh stack unstack`](#gh-stack-unstack)   | Remove a stack from local tracking and unstack it on GitHub.                              |
| [`gh stack submit`](#gh-stack-submit)     | Push all branches, then create or update pull requests and the stack on GitHub.           |
...
| [`gh stack link`](#gh-stack-link)         | Link pull requests into a stack on GitHub without local tracking.                         |
...
If the branches already have open pull requests but no stack exists on GitHub, you have the option to link the pull requests into a stack with <kbd>Ctrl</kbd>+<kbd>B</kbd>.
...
Creates or updates a stack on GitHub from branch names or pull request numbers and URLs. This command does not create or modify any local tracking state. It is designed for people who manage branches with other tools locally, such as Jujutsu, Sapling, or git-town, and want to open a stack of pull requests. See [Use other tools with stacked pull requests](/en/pull-requests/reference/use-other-tools-with-stacked-pull-requests).
