...
* **GraphQL**: Issue field types and mutations are also available via GraphQL. See the [`IssueFields` union](/en/graphql/reference/unions#issuefields), [issue field objects](/en/graphql/reference/objects#issuefieldtext) (such as `IssueFieldText`, `IssueFieldSingleSelect`, `IssueFieldNumber`, and `IssueFieldDate`), and [mutations](/en/graphql/reference/mutations#createissuefield) (such as `createIssueField`, `updateIssueField`, and `setIssueFieldValue`).