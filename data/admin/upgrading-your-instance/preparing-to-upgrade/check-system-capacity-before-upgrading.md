...
Before upgrading GitHub Enterprise Server, you should perform these capacity checks and take the recommended steps.

Upgrading to newer versions of GitHub Enterprise Server typically increases resource consumption. Each feature release adds new functionality, some enabled by default, others opt-in, which requires more processing power. Customer usage patterns also affect demand; for example, enterprises with tens of thousands of organizations may see higher resource usage.
...
1. **Check CPU utilization.**
   In the Management Console, go to the monitor page (`https://HOSTNAME.com:8443/setup/monitor`) and view the `CPU` graph.
...
1. **Check memory usage.**
    In the Management Console, go to the monitor page (`https://HOSTNAME.com:8443/setup/monitor`) and view the `Memory` graph.
...
1. **Check disk graphs in the Management Console.**
...
1. **Check the queued requests graph.**
   In the Management Console, go to the monitor page (`https://HOSTNAME.com:8443/setup/monitor`) and view the `Queued Requests` graph.
...
1. **Check the Aqueduct queue depth.**
   In the Management Console, go to the monitor page (`https://HOSTNAME.com:8443/setup/monitor`) and view the `Aqueduct queue depth` graph.
...
* Example: (280 – 100) ÷ 12 = 15 additional workers needed.
  ...
  >[!TIP] If you want to confirm your findings, you can reach out to us by visiting [GitHub Enterprise Support](https://support.github.com), uploading a bundle, and asking for the total target number of unicorn workers.
