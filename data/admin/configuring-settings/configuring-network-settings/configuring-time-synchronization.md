...
You must ensure that the NTP servers (configured or default) are reachable on UDP port 123. For more details on the network ports that need to be open, see [Network ports](/en/enterprise-server@3.20/admin/configuring-settings/configuring-network-settings/network-ports#administrative-ports).
