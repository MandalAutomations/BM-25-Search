...
Find information about high availability replica types, communication patterns, maintenance operations, and how to choose the right replica for your deployment.
...
**Passive replicas** simply sync data from the primary instance and don't handle any GitHub traffic. However, operators can promote a passive replica to primary if needed.

A **geo replica** is an example of an **active replica** (these terms are often used interchangeably). Active replicas sync the data from the primary. An active replica can also process GitHub traffic directly or proxy them to the primary.
...
Primary and active replicas have strict latency requirements. Primary and passive replicas, as well as primary and cache replicas, have recommended latency requirements. For more information, see [Creating a high availability replica](/en/enterprise-server@3.20/admin/monitoring-and-managing-your-instance/configuring-high-availability/creating-a-high-availability-replica#creating-geo-replication-replicas) and [Monitoring a high-availability configuration](/en/enterprise-server@3.20/admin/monitoring-and-managing-your-instance/configuring-high-availability/monitoring-a-high-availability-configuration#communication-issues-between-nodes). Network latencies beyond the required and recommended values may cause replicas to constantly lag behind.

## Administrative access and monitoring

### Is the Management Console available on replicas?

The Management Console is not available on either passive replicas or cache replicas. It is available only on active replicas (active replicas forward most of the requests to the primary).

### Is it possible to SSH into replicas?

An operator with administrative shell access can SSH into any of the replicas. Operators can add their public keys to the new replica via the Management Console before the replica is added to the cluster. For more information, see [Accessing the administrative shell (SSH)](/en/enterprise-server@3.20/admin/administering-your-instance/administering-your-instance-from-the-command-line/accessing-the-administrative-shell-ssh).
...
All replicas can be monitored. The Management Console on the primary instance provides dashboards for all nodes, including passive and active replica nodes in the deployment.

In addition, you can export metrics and logs from all the nodes in a deployment to third party monitoring platforms.

To learn how to monitor the status of data replication among replica nodes, see [Monitoring a high-availability configuration](/en/enterprise-server@3.20/admin/monitoring-and-managing-your-instance/configuring-high-availability/monitoring-a-high-availability-configuration#communication-issues-between-nodes).
...
Backups are used to create copies of your data that can be restored to another GitHub Enterprise Server environment. Customers often use backups to recover from disasters, or create new installations. In short, backup data is used to restore another GitHub Enterprise Server instance, while replicas are designed for high availability and redundancy in real time.

Replicas themselves are GitHub Enterprise Server instances. Backup-host is not a GitHub Enterprise Server installation.

### What software is running on replicas?

Replicas are a separate installation of GitHub Enterprise Server. The primary instance and all replicas should be running the same version of GitHub Enterprise Server.
...
By design, cache replicas come with trade-offs. Cache replicas are eventually consistent and don't always serve the latest repository content. However, there are webhooks for when the latest changes land on the replica so the relevant CI/CD jobs can be kicked off. Very few GitHub customers use cache replicas.