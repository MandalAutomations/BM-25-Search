# Adding nodes to a high availability configuration

Add nodes to the primary high availability (HA) datacenter. This is intended to offload CPU-intensive tasks from the primary node, allowing for horizontal scaling of the GitHub Enterprise Server instance.

> [!NOTE]
> The ability to add additional compute nodes to HA is in public preview and subject to change. During the preview, please share any feedback with your customer success team.

For GitHub Enterprise Server customers looking to scale horizontally, migrating to and operating a cluster is an option, but is resource-intensive and time-consuming. As an alternative, we recommend adding nodes to an HA configuration.
